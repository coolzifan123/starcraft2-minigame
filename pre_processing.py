...
map_dict=dict()
map_dict['MoveToBeacon']=[7,331]
map_dict['CollectMineralShards']=[7,331]
map_dict['DefeatRoaches']=[0,12,7]
map_dict['FindAndDefeatZerglings']=[1,12]
map_dict['DefeatZerglingsAndBanelings']=[0,2,12]
map_dict['CollectMineralsAndGas']=[0,2,44,91,264,490]
map_dict['BuildMarines']=[0, 2, 42, 91,264,477,490]
class Preprocessor():
  ...
  def __init__(self, obs_spec,map,process):
    self.mapdict=map_dict
  ...
  def _preprocess_obs(self, obs):
    ...
    #unmask = np.intersect1d(obs['available_actions'],np.array([0,2,7,331]))
    #unmask = np.intersect1d(obs['available_actions'], np.array([0, 2, 42, 91,264,477,490]))
    #unmask = np.intersect1d(obs['available_actions'], np.array([0,12,7]))
    unmask = np.intersect1d(obs['available_actions'], np.array(self.mapdict[self.map]))
    # if len(unmask) > 1:
    #     unmask = np.delete([0])
